...
describe('ProgressTracker', () => {
  ...
  describe('getStats', () => {
    ...
    it('reflects team invites', () => {
      wizardStore.getOrCreate('t1');
      wizardStore.addTeamInvites('t1', [
  ...
  describe('getCompletionPercent', () => {
    it('returns 0 for fresh tenant', () => {
      expect(tracker.getCompletionPercent('t1')).toBe(0);
    });

    it('increases as steps complete', () => {
      wizardStore.getOrCreate('t1');
      wizardStore.completeStep('t1', 'welcome');
      expect(tracker.getCompletionPercent('t1')).toBeGreaterThan(0);
    });
  });
});
