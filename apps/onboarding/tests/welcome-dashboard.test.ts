...
describe('WelcomeDashboardService', () => {
  ...
  describe('getDashboard', () => {
    it('returns dashboard for new tenant', async () => {
      const dashboard = await welcome.getDashboard('t1');
    ...
    it('nextStep is null when onboarding complete', async () => {
      ...
      for (const step of steps) {
        await wizardStore.completeStep('t1', step);
      }
      const dashboard = await welcome.getDashboard('t1');
    ...
    it('quick action add_feed is completed when feeds active', async () => {
      await wizardStore.getOrCreate('t1');
  ...
  describe('shouldShowWelcome', () => {
    it('returns true for incomplete onboarding', async () => {
      await wizardStore.getOrCreate('t1');
      expect(await welcome.shouldShowWelcome('t1')).toBe(true);
    });

    it('returns false when onboarding complete', async () => {
      await wizardStore.getOrCreate('t1');
      const steps = [
        'welcome', 'org_profile', 'team_invite', 'feed_activation',
        'integration_setup', 'dashboard_config', 'readiness_check', 'launch',
      ...
      expect(await welcome.shouldShowWelcome('t1')).toBe(false);
    });
  });
