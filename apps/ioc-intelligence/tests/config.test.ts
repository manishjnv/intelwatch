...
describe('IOC Intelligence — Config', () => {
  const validEnv = {
  ...
  it('loads config with valid env vars', () => {
    ...
    expect(config.TI_IOC_INTELLIGENCE_PORT).toBe(3007);
    expect(config.TI_IOC_INTELLIGENCE_HOST).toBe('0.0.0.0');
    expect(config.TI_NODE_ENV).toBe('development');
    expect(config.TI_LOG_LEVEL).toBe('info');
  });

  it('applies custom port from env', () => {
    const config = loadConfig({ ...validEnv, TI_IOC_INTELLIGENCE_PORT: '4007' });
    expect(config.TI_IOC_INTELLIGENCE_PORT).toBe(4007);
  });
