...
async function main(): Promise<void> {
  ...
  try {
    const address = await app.listen({ port: config.TI_IOC_INTELLIGENCE_PORT, host: config.TI_IOC_INTELLIGENCE_HOST });
    logger.info(`ETIP IOC Intelligence Service listening at ${address}`);
  } catch (err) {
    logger.fatal({ err }, 'Failed to start IOC Intelligence Service');
    process.exit(1);
  }
