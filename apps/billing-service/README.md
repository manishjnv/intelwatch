...
**Port:** 3019 | **Phase:** 6 | **Status:** 🔨 WIP | **Tests:** 157
...
| Razorpay Integration | `services/razorpay-client.ts` | Customer, subscription, order, webhook signature verification |
| Invoice & Billing | `services/invoice-store.ts` | Monthly invoices with GST (18%) + receipts |
| Upgrade/Downgrade | `services/upgrade-flow.ts` | Proration, scheduled downgrade, grace period |
| Coupon Codes (P0 #10) | `services/coupon-store.ts` | Percentage/flat discounts with expiry + max-uses |
| Upgrade Prompts (P0 #6) | `routes/p0-features.ts` | Contextual prompts at 80%/90%/100% usage |
| Usage Alerts (P0 #7) | `routes/p0-features.ts` + `services/usage-store.ts` | Threshold alerts (80/90/100%) |
| Grace Period (P0 #8) | `services/upgrade-flow.ts` | 72-hour tolerance after limit hit |
| Billing Dashboard (P0 #9) | `routes/admin.ts` | Revenue, MRR, churn, plan distribution |
...
| POST | /api/v1/billing/upgrade/downgrade | Schedule downgrade |
...
| POST | /api/v1/billing/webhooks/razorpay | Razorpay webhook (HMAC verified) |
...
| Plan | Price (INR) | Price (USD) | IOC Queries/day | Storage | Feeds | Users |
|------|-------------|-------------|-----------------|---------|-------|-------|
| Free | ₹0 | $0 | 100 | 10K | 3 | 2 |
| Starter | ₹4,999/mo | $59/mo | 5,000 | 500K | 20 | 10 |
| Pro | ₹14,999/mo | $179/mo | 50,000 | 5M | Unlimited | 50 |
...
- **GST:** 18% automatically added to all invoices.
- **Grace period:** 72 hours after plan limit exceeded before hard cutoff.
- **Webhooks:** HMAC-SHA256 verified before any processing.
