...
const ConfigSchema = z.object({
  ...
  TI_SERVICE_PORT: z.coerce.number().int().min(1).max(65535).default(3021),
  ...
  TI_REPORT_RETENTION_DAYS: z.coerce.number().int().min(1).default(30),
  TI_REPORT_MAX_PER_TENANT: z.coerce.number().int().min(10).default(100),
});

export type ReportingConfig = z.infer<typeof ConfigSchema>;

let _config: ReportingConfig | null = null;

export function loadConfig(env: Record<string, string | undefined>): ReportingConfig {
...
export function getConfig(): ReportingConfig {
