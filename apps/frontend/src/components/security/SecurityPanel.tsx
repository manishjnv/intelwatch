...
export function MfaEnforcementCard({ scope }: { scope: 'tenant' | 'platform' }) {
  ...
  const handleToggle = (isEnforced: boolean) => {
  ...
  return (
    <QueryStateView
      query={enforcement}
      resource="MFA enforcement"
      skeleton={<div className="h-16 bg-bg-elevated rounded-xl animate-pulse" />}
    >
      {data => {
        const isEnforced = data.enforced
        ...
        return (
          <div className="border border-border rounded-xl p-5 bg-bg-primary" data-testid="mfa-enforcement-card">
            <div className="flex items-center justify-between gap-4">
              ...
              <button
                onClick={() => handleToggle(isEnforced)}
                disabled={pending}
