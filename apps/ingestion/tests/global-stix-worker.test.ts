...
describe('GlobalSTIXWorker', () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it('uses correct queue name: FEED_FETCH_GLOBAL_STIX', () => {
    createGlobalSTIXWorker({ db: {} as never, logger: createMockLogger(), redisUrl: 'redis://localhost:6379' });
    const workerCtor = Worker as unknown as ReturnType<typeof vi.fn>;
    expect(workerCtor.mock.calls[0][0]).toBe(QUEUES.FEED_FETCH_GLOBAL_STIX);
  });
