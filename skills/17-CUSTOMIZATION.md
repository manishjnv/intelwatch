# SKILL: Platform Customization
**ID:** 17-customization | **Version:** 3.0
...
## MODULE DESCRIPTION
TenantCustomization schema: branding (logo/colors/name), module toggles, feature flags, risk score weights, alert thresholds, custom taxonomy, field visibility, IP whitelist. FeatureFlagService: Redis-cached (1min TTL), used everywhere. Module activation middleware: requireModule() on routes. Custom risk scoring: per-tenant weights (VT/AbuseIPDB/AI), custom severity thresholds, per-type boosts. Alert quiet hours. AI model selection per use case (see skill 22-admin-platform). Full settings UI at /settings with sub-pages for each category. All customization changes → audit log. White-label: custom logo/colors/platform name shown in header.

## FILE STRUCTURE (max 400 lines per file)
```
/apps/17-customization-service/src/
