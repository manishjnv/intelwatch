# SKILL: Threat Hunting Workspace
**ID:** 14-hunting | **Version:** 3.0
...
## MODULE DESCRIPTION
Analyst investigation workspace. Investigation schema: title, hypothesis, status, severity, assignedTo, entities, timeline (ordered events), findings (markdown), tags. Pivot query: multi-hop entity traversal via graph service. AI next-step suggestions: given current investigation state, Claude suggests 3-5 specific hunting actions. Collaboration: share investigation with team, comment thread. Evidence export: generate PDF report from investigation. Entity click anywhere in hunting workspace → opens InvestigationPanel (see skill 20-ui-ux). Timeline visualization with zoom. Integration: create ITSM ticket from investigation when integration configured.

## FILE STRUCTURE (max 400 lines per file)
```
/apps/14-hunting-service/src/
