# SKILL: Free to Paid Adoption
**ID:** 19-free-to-paid | **Version:** 3.0
...
## MODULE DESCRIPTION
Tiers: Free (100 IOC queries/day, 10K storage, 3 feeds, 2 users) / Starter 99 (5K queries, 500K storage, 20 feeds, 10 users, 3 integrations) / Professional 99 (50K queries, 5M storage, unlimited feeds, 50 users, graph viz, dark web) / Enterprise (custom, everything unlimited). Plan limits enforced via middleware — 429 response includes upgradeUrl. Smart upgrade triggers: fire at moment of maximum perceived value (not randomly). FeatureGate React component wraps premium features. 14-day trial management. Stripe checkout + webhook. Usage dashboard shows consumption % per metric. Upgrade recommended automatically when >80% of any limit.

## FILE STRUCTURE (max 400 lines per file)
```
/apps/19-free-to-paid-service/src/
