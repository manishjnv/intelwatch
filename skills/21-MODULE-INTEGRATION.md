...
**ID:** 21-module-integration | **Version:** 3.0
...
## MODULE DESCRIPTION
All inter-service communication via REST APIs (sync) or BullMQ queues (async). Service registry: env vars per service URL. Internal service client: signed service JWT, 10s timeout, retry on 5xx. Event bus (BullMQ+Redis): canonical event types (ioc.created/enrichment.complete/severity.high/correlation.match/alert.triggered). Event flow: IOC created → enrichment queue → enrichment done → correlation queue → notifications queue → integration queue → WebSocket push to UI. API gateway: all external traffic enters here, auth enforced, routes to correct service. WebSocket: real-time UI updates via Socket.IO (tenant room isolation). Integration test: full pipeline test from raw IOC ingest to ITSM ticket creation.

## FILE STRUCTURE (max 400 lines per file)
```
/apps/21-module-integration-service/src/
