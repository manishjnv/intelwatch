# SKILL: Digital Risk Protection
**ID:** 11-drp | **Version:** 3.0
...
## MODULE DESCRIPTION
Monitor external attack surface. Detection: phishing domains, typosquatting (homoglyph + insertion + deletion + transposition + TLD variants), credential leaks (HIBP integration), fake social media, rogue mobile apps, dark web brand mentions. MonitoredAsset schema: domain/brand_name/email_domain/social_handle/mobile_app. DRPAlert schema: type, severity, evidence, status (open/investigating/resolved/false_positive). AI enrichment per alert: hosting provider, takedown contact, recommended actions. Takedown request generation (template + contact lookup). Tenant configures monitored assets from settings panel.

## FILE STRUCTURE (max 400 lines per file)
```
/apps/11-drp-service/src/
