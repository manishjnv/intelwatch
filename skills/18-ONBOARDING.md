...
## MODULE DESCRIPTION
8-step wizard: welcome survey → org setup → team invite → feed activation → integration setup (optional, skippable) → dashboard customization → readiness check → launch celebration. Demo data seeded immediately at signup (150 IOCs, 10 actors, 20 malware, 50 CVEs, 5 DRP alerts) — all tagged as DEMO so users can distinguish. Readiness check: automated validation (feeds pulling? first IOC ingested? enrichment working? team invited?). Contextual help at each step (tooltip + video link). Email drip sequence (signup → verify → 24h nudge → complete → 3d tips → 7d upgrade). Onboarding state machine (step/completed/skipped). Can restart onboarding from settings. Progress shown in sidebar until complete.

## FILE STRUCTURE (max 400 lines per file)
```
/apps/18-onboarding-service/src/
