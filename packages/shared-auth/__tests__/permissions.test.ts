/**
 * @module @etip/shared-auth/__tests__/permissions.test
 * @description Tests for RBAC permission checking.
 */
...
describe('RBAC Permissions', () => {
  describe('hasPermission', () => {
    ...
    // ── tenant_admin (resource wildcards) ──
    ...
    it('tenant_admin does NOT have admin:write', () => {
      expect(hasPermission('tenant_admin', 'admin:write')).toBe(false);
    });
    ...
    it('analyst cannot delete IOCs', () => {
      expect(hasPermission('analyst', 'ioc:delete')).toBe(false);
    });
  ...
  describe('hasAllPermissions', () => {
    it('returns true when role has all required permissions', () => {
      expect(
        hasAllPermissions('analyst', ['ioc:read', 'ioc:create', 'dashboard:read'])
      ).toBe(true);
    });

    it('returns false when role is missing one permission', () => {
      expect(
        hasAllPermissions('viewer', ['ioc:read', 'ioc:create'])
      ).toBe(false);
    });
  ...
  describe('hasAnyPermission', () => {
    it('returns true when role has at least one required permission', () => {
      expect(
        hasAnyPermission('viewer', ['ioc:create', 'ioc:read'])
      ).toBe(true);
    });

    it('returns false when role has none of the required permissions', () => {
      expect(
        hasAnyPermission('api_only', ['dashboard:read', 'admin:write'])
      ).toBe(false);
    });
  ...
  describe('getResolvedPermissions', () => {
    it('returns wildcard for super_admin', () => {
      const perms = getResolvedPermissions('super_admin');
  ...
  describe('ROLE_PERMISSIONS completeness', () => {
    it('has entries for all defined roles', () => {
      const expectedRoles = ['super_admin', 'tenant_admin', 'analyst', 'viewer', 'api_only'];
  ...
  describe('PERMISSIONS constants', () => {
    it('has expected permission keys', () => {
      ...
      expect(PERMISSIONS.IOC_CREATE).toBe('ioc:create');
