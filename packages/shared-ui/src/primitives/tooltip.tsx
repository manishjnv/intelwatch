...
interface TooltipProps { children: React.ReactNode; placement?: Placement; delayMs?: number }
export function Tooltip({ children, placement = 'top', delayMs = 300 }: TooltipProps) {
  ...
  const value = useMemo(() => ({ open, refs, floatingStyles, getFloatingProps, getReferenceProps }),
    [open, refs, floatingStyles, getFloatingProps, getReferenceProps])
  return <TooltipContext.Provider value={value}>{children}</TooltipContext.Provider>
}
interface TooltipTriggerProps { children: React.ReactElement; asChild?: boolean }
export function TooltipTrigger({ children, asChild }: TooltipTriggerProps) {
  ...
  return <span ref={refs.setReference} {...getReferenceProps()}>{children}</span>
}
interface TooltipContentProps { children: React.ReactNode; className?: string }
