...
// Entity type colors, pill shape, hover actions, and icon sizes are FROZEN.
...
// ⛔ FROZEN — entity type → Tailwind class mapping (UI_DESIGN_LOCK.md)
...
// ⛔ FROZEN — external search URLs per entity type (UI_DESIGN_LOCK.md)
...
// ⛔ FROZEN — pill shape: px-2 py-0.5 rounded border text-xs (UI_DESIGN_LOCK.md)
// ⛔ FROZEN — hash mono: font-mono text-[10px] max-w-[120px] truncate
// ⛔ FROZEN — icon size: w-3 h-3 (never larger)
export function EntityChip({
  ...
  const handleInternetSearch = (e: React.MouseEvent) => {
    e.stopPropagation()
    window.open(INTERNET_SEARCH_URLS[type](value), '_blank', 'noopener')
  }

  return (
    <Popover>
      <PopoverTrigger asChild>
        {/* ⛔ FROZEN pill classes below */}
        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded border text-xs cursor-pointer transition-all ${cfg.bg} ${cfg.text} ${cfg.border} hover:opacity-80`}>
          {/* ⛔ FROZEN icon: w-3 h-3 always */}
          <span className="w-3 h-3 shrink-0" aria-hidden />
          {/* ⛔ FROZEN hash mono truncate */}
          <span className={isHash ? 'font-mono text-[10px] max-w-[120px] truncate' : ''}>
            {value}
          </span>
          {severity && (
            // ⛔ FROZEN severity dot
            <span className={`w-1.5 h-1.5 rounded-full ${SEVERITY_DOT[severity]} shrink-0`} />
          )}
        </span>
      </PopoverTrigger>

      <PopoverContent className="w-72 bg-[var(--bg-elevated)] border-[var(--border)] shadow-lg p-3 z-50">
        <div className="flex flex-col gap-2">
          ...
          {/* ⛔ FROZEN 6-action set (UI_DESIGN_LOCK.md) */}
          <div className="grid grid-cols-3 gap-1 pt-1 border-t border-[var(--border)]">
            {showCopy && (
              <button onClick={handleCopy} className="flex items-center gap-1 text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] px-2 py-1 rounded hover:bg-[var(--bg-hover)]">
            ...
            <button onClick={handleInternetSearch} className="flex items-center gap-1 text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] px-2 py-1 rounded hover:bg-[var(--bg-hover)]">
              <Globe className="w-3 h-3" /> Online
            </button>
            <button onClick={() => onInvestigate?.(type, value)} className="flex items-center gap-1 text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] px-2 py-1 rounded hover:bg-[var(--bg-hover)]">
              <Plus className="w-3 h-3" /> Investigate
            </button>
            <button onClick={() => onGraph?.(type, value)} className="flex items-center gap-1 text-[10px] text-[var(--text-secondary)] hover:text-[var(--text-primary)] px-2 py-1 rounded hover:bg-[var(--bg-hover)]">
