...
**Session counter:** 142 — S142: IOC Intelligence Tier 3 — 5 differentiator features (confidence decay chart, MITRE badges, risk propagation banner, IOC compare panel, inline enrichment expansion). 12 new files, 7 modified, 81 new tests. 1,708 frontend tests. Deployed.
...
| etip_frontend | ✅ Running | 0.24.0 | 2026-04-02 | Dashboard + 20 data pages + Command Center (10 tabs SA / 7 TA). **Session 142:** IOC Tier 3 — Confidence decay chart, MITRE badges, risk propagation banner, IOC compare panel, inline enrichment expansion. 1,708 tests. |
...
| frontend | 1 | ✅ UI FROZEN | 2026-04-02 | **24 data pages + Command Center (10 tabs SA / 7 TA)**. 1,708 tests. **Session 142:** IOC Tier 3 — Confidence decay chart (SVG, DECISION-015 rates), MITRE ATT&CK badges (table+detail), Risk propagation banner, IOC compare panel (2-3 column diff), Inline enrichment expansion (DataTable expandable rows). 12 new components, 81 new tests. |
...
- **Current phase:** Phase 12 — Command Center v2.1 (Protection & Hardening). Sessions 111-142.
- **Last session outcome:** Session 141 (2026-04-02). **S141: IOC Intelligence Page — Tier 3 (Best-in-Class).** 5 features: (1) Confidence Decay Chart — animated SVG line chart with exponential decay per DECISION-015, type-specific half-lives, event markers. (2) MITRE ATT&CK TTP Badges — compact technique IDs color-coded by tactic, tooltip with full name, MitreDetailSection in detail panel. (3) Retroactive Risk Propagation Banner — animated amber banner showing correlation events per DECISION-020, dismissible. (4) IOC Quick Compare — full-screen modal comparing 2-3 IOCs side-by-side, diff highlighting on differing fields. (5) Expandable Inline Enrichment Rows — accordion rows in DataTable showing VT ratio, AbuseIPDB score, geo flag, risk verdict. DataTable enhanced with expandableRow/expandedRowId/onExpandRow props. Commits: e9a071c (feat) + d179a22 (lint fix). CI run 23903961558 green. Deployed. 1,708 frontend tests (106 files, +49 new).
