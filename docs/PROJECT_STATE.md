...
**Session counter:** 116 — SCIM 2.0 Provisioning (I-12) + Free-to-Paid Billing Upgrade (I-14): SCIM token management, /Users CRUD, /Groups read-only, bearer token auth, deprovisioning with session termination + API key revocation; POST /billing/upgrade with downgrade protection, GET /billing/plans. 49 new tests. Pushed to master. Commit 48798de.
...
| etip_user_management | ✅ Deployed | 0.3.0 | 2026-03-30 | Port 3016. 38 endpoints, 259 tests. RBAC + teams + SSO + MFA + break-glass + SCIM 2.0. **Session 116 (I-12):** SCIM token management, /Users CRUD, /Groups read-only, bearer token auth, deprovisioning (session termination + API key revocation). |
...
| user-management | 5 | 🔨 WIP | 2026-03-30 | Port 3016. **Core + 5 P0 + I-03/I-04/I-05 + I-11/I-12/I-13 COMPLETE**. 38 endpoints, 259 tests. RBAC (15 resources, 5 built-in roles). Team mgmt. SSO config (SAML 2.0 + OIDC per-tenant). MFA (TOTP + backup codes + enforcement). Break-glass. **I-12:** SCIM 2.0 provisioning — ScimToken model, /Users CRUD (create/read/update/deprovision), /Groups read-only, bearer token auth, deprovisioning terminates sessions + revokes API keys. 49 new tests. FEATURE-COMPLETE. |
