# ETIP Project State
**Last updated:** 2026-09-22 (update at end of EVERY session via /session-end)
**Session counter:** 146 — S146: VPS stabilised (ES/Neo4j memory fix live, 32/32 healthy). CI blocker found + fixed. VPS hardening + SEO Phase 1 committed on branch `feat/vps-hardening-seo-phase1`, NOT yet deployed.
...
- **Last session outcome:** Session 146 (2026-09-22). Project review + SEO market research → docs/SEO_PLAN.md. **Deployed (master a22aa9e):** ES 1G→2G, Neo4j 768M→2G + heap/pagecache, Neo4j tmpfs /tmp, dhanradar Prometheus scrape job brought into git. ES was OOM-killed (353 restarts) and Neo4j had 404,378 restarts; both now healthy, 0 restarts, ~1.2G/1.0G used. **On branch `feat/vps-hardening-seo-phase1`, NOT deployed:** 0a210fb offboarding test clock pin (the CI blocker), 2ac97a5 all 33 published ports bound to 127.0.0.1 + nginx security headers + noindex on api/grafana/health/ws, cfce906 SEO Phase 1 (robots.txt with AI crawlers disallowed, sitemap.xml, apps/frontend/nginx.conf with true 404s + noindex on SPA routes, canonical/OG PNG/twitter/JSON-LD, all non-landing routes lazy-loaded — landing entry chunk now 264 kB / 84 kB gzip).
- **Known issues:** (1) **CI on master is red** until 0a210fb merges — any push to master will not deploy. (2) apps/frontend/nginx.conf and the outer nginx changes have NOT been run in a real nginx yet (local docker test was declined) — validate with `nginx -t` before/at deploy. (3) Frontend vitest cannot run on the local machine: Node 20.11.1 is too old for jsdom 29 (ERR_REQUIRE_ESM) — CI (Node 20 latest) is the only place frontend tests run. (4) user-management `session-ttl-isolation-apikey` beforeEach hook times out at 10s when the whole monorepo runs in parallel locally (load, not logic). (5) 26 frontend test files contain hardcoded 2025/2026 ISO dates — more time bombs possible. (6) Local shell has an invalid `GH_TOKEN` env var: run `env -u GH_TOKEN gh …` / `env -u GH_TOKEN git push`. (7) VPS: host free memory ~3G after the limit raise; ufw inactive (only sshd:22 listens publicly, key-only; Hostinger edge filters the rest on IPv4, IPv6 unverified until 2ac97a5 is live). (8) Compose still has a default Grafana password fallback (VPS .env sets a real one). (9) GitHub Actions still on checkout@v4/setup-node@v4/pnpm action-setup@v4 (forced onto Node 24, works, warns). Carried over from S145: Shodan/GreyNoise/GSB/IPinfo keys unset on VPS; alert fan-out in-memory tenant registry; InvestigationDrawer demo data; POST /api/v1/iocs missing; bulk re-enrichment missing; IOC `source`/`hasCampaign` filters unsupported; 3 REST feeds not fetching.
- **Next tasks:** (1) Open/merge PR for `feat/vps-hardening-seo-phase1` once CI is green on it; watch the deploy; on VPS run `docker exec etip_nginx nginx -t` and the curl grid in docs/SESSION_HANDOFF.md; confirm site still loads through the tunnel after ports move to 127.0.0.1 (rollback: `git tag safe-point-2026-09-22-vps-seo`, VPS files in /root/backup-20260922). (2) Off-repo SEO setup: Google Search Console + Bing Webmaster verification, submit sitemap, Cloudflare www→apex rule, check Cloudflare AI-bot/managed robots settings. (3) SEO Phase 2 (prerender + /pricing, /features, /glossary) — owner approved additive api-gateway routes, new marketing pages in the frozen frontend, INR-only pricing, AI crawlers blocked. (4) Disk-usage alert (needs a notification channel). (5) Backlog from S145 list above.
