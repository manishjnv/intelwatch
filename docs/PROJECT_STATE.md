...
- **Next tasks:** (1) Owner: click-through test of the logged-in app; Google Search Console + Bing Webmaster verification, submit sitemap, Cloudflare www→apex + AI-bot settings. (2) SEO 2c: `/features/*` + `/solutions/*` + about/contact/legal pages (reuse `PublicLayout`, add to `PUBLIC_ROUTES`). (3) SEO 2d: glossary + sitemap generated from `PUBLIC_ROUTES`. (4) Wire Command Center Clients to real tenants / the plan-change endpoint. (5) Cache auth verify results; session revocation check. (6) Self-serve Razorpay checkout — needs its own decision.
