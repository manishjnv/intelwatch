...
| etip_frontend | ✅ Running | 0.8.0 | 2026-03-25 | Dashboard + 20 data pages + demo fallbacks (all 5 entity types + reporting + alerting + analytics + IOC search). E2E C2-D2: correlation actions, DRP triage, IOC pivot/timeline, alerting fixes, AnalyticsPage. E2 queue health: AdminOpsPage queue table (color-coded, 10s polling). F3: AI Config tab rebuilt — plan selector (4 tiers), 12-subtask table, cost estimator sidebar. G5: SearchPage (/search) — full-text IOC search, 3 filters, ES demo fallback. AdminOpsPage: DLQ table (retry/discard/retry-all). 704 frontend tests (706 total, 2 skipped). |
...
| etip_admin | ✅ Deployed | 0.2.0 | 2026-03-25 | Port 3022. 33 endpoints, 172 tests (+11 queue-monitor, +10 DLQ processor). System health monitoring, maintenance windows, backup/restore, tenant administration, audit dashboard + 5 P0 improvements. Queue monitor: GET /admin/queues (live BullMQ depth via ioredis). DLQ processor: GET /admin/dlq (failed counts), POST /admin/dlq/:queue/retry, POST /admin/dlq/:queue/discard, POST /admin/dlq/retry-all. |
...
| admin-ops | 6 | ✅ Complete | 2026-03-25 | Port 3022. **Core + 5 P0 improvements + queue monitor + DLQ processor COMPLETE**. 33 endpoints, 172 tests. System health (18 services), maintenance windows (CRUD + activate/deactivate), backup/restore, tenant admin (CRUD + suspend/reinstate/plan/usage), audit log (CSV export). P0: dependency map, alert rules (seeded 5 defaults), scheduled maintenance (cron), tenant analytics, admin activity log. Queue monitor: GET /admin/queues reads 14 BullMQ queues via ioredis LLEN+ZCARD. DLQ processor: GET /dlq (failed counts), POST /dlq/:queue/retry (ZRANGE+ZREM+LPUSH), POST /dlq/:queue/discard (DEL), POST /dlq/retry-all. FEATURE-COMPLETE. **Phase 6 COMPLETE (3/3).** |
