...
**Session counter:** 116 — SSO Group-to-Role Mapping (I-11) + Email Verification (I-13): SsoConfig Prisma model, SAML/OIDC config CRUD, group-to-role mapping, JIT provisioning, domain validation, email verification flow (token gen/verify/resend/cleanup), login guard, 22 new tests. Pushed to master.
...
| user-service | 1 | ✅ Deployed | 2026-03-29 | **Session 116:** SSO group-to-role mapping (I-11) + email verification (I-13). SsoConfig model, JIT provisioning, domain validation. Email verify/resend/cleanup. Login guard. 73 tests. |
...
| user-management | 5 | 🔨 WIP | 2026-03-29 | Port 3016. **Core + 5 P0 + I-03/I-04/I-05 + I-11/I-13 COMPLETE**. 33 endpoints, 210 tests. RBAC (15 resources, 5 built-in roles). Team mgmt. SSO config (SAML 2.0 + OIDC per-tenant). MFA (TOTP + backup codes + enforcement). Break-glass. P0: permission inheritance, SOC2 audit trail, brute-force protection, session management, password policy. **I-11:** SSO group-to-role mapping, JIT provisioning, domain validation, SsoConfig Prisma model. **I-13:** Email verification flow (token gen, verify, resend, cleanup, login guard). FEATURE-COMPLETE. |
...
- **Last session outcome:** Session 116 (2026-03-29). **SSO Group-to-Role Mapping (I-11) + Email Verification (I-13) COMPLETE.** I-11: SsoConfig Prisma model (SAML/OIDC per-tenant), group-to-role mapping validation, JIT provisioning, domain validation, maxUsers enforcement, MFA enforcement passthrough. I-13: Email verification token gen (SHA-256 hashed, 24h expiry), verify/resend/cleanup endpoints, login guard (403 if unverified), rate-limited resend (5min), 7-day unverified cleanup. 22 new tests (73 total user-service). Commit ecb327f. Pushed to master. ~7,262 monorepo tests.
- **Known issues:** Shodan/GreyNoise API keys not set on VPS (enrichment degrades gracefully). Alert fan-out uses in-memory tenant registry. 2 pre-existing test files fail (batch-normalizer, fuzzy-dedupe-integration) due to vitest alias caching. VPS needs `prisma db push` (SsoConfig model + User email verification fields + plan models + migration 0003). 1 pre-existing flaky test in shared-auth (password.test.ts unique salts).
- **Next tasks:** (1) Run `prisma db push` on VPS for SsoConfig + email verification + plan models + migration 0003. (2) Run seed script for 4 default plans. (3) Set TI_MFA_ENCRYPTION_KEY env var on VPS. (4) Command Center v2.1 — remaining I-items or Quota UI (billing/limits frontend). (5) Set Shodan/GreyNoise API keys on VPS.
