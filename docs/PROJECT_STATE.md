...
**Session counter:** 117 — Access Review Automation (I-17) + Compliance Report Generation (I-18): Stale super_admin/user scans, 14-day auto-disable, review confirm/disable endpoints, quarterly summary, SOC 2 report, privileged access report, GDPR DSAR export. 11 endpoints, 20 new tests. Pushed to master. Commit 4774489.
...
| etip_user_management | ✅ Deployed | 0.4.0 | 2026-03-30 | Port 3016. 49 endpoints, 279 tests. RBAC + teams + SSO + MFA + break-glass + SCIM 2.0 + access review + compliance reports. **Session 117 (I-17, I-18):** Stale super_admin/user scans, auto-disable, review endpoints, quarterly summary, SOC 2 / privileged access / GDPR DSAR reports. 11 new endpoints, 20 new tests. |
...
| shared-utils | 1 | ✅ Deployed | 2026-03-29 | QUEUES: 25 constants (+EMAIL_SEND). EVENTS: 22 constants (+GLOBAL_FEED_PROCESSED, +GLOBAL_IOC_CREATED). **registerMetrics()**: prom-client Prometheus plugin. 91 tests. |
...
| user-management | 5 | 🔨 WIP | 2026-03-30 | Port 3016. **Core + 5 P0 + I-03/I-04/I-05 + I-11/I-12/I-13 + I-17/I-18 COMPLETE**. 49 endpoints, 279 tests. RBAC (15 resources, 5 built-in roles). Team mgmt. SSO config. MFA. Break-glass. SCIM 2.0. **I-17:** Stale super_admin scan (60d), stale user scan (90d), 14-day auto-disable, review confirm/disable, quarterly summary. **I-18:** SOC 2 access review, privileged access, GDPR DSAR export. 11 new endpoints, 20 new tests. |
...
- **Current phase:** Phase 12 — Command Center v2.1 (Protection & Hardening). Sessions 111-116.
- **Last session outcome:** Session 116 (2026-03-30). **SCIM 2.0 Provisioning (I-12) + Free-to-Paid Billing Upgrade (I-14) COMPLETE.** I-12: ScimToken Prisma model, SCIM bearer token management (generate/list/revoke), /Users CRUD (create/patch/delete with session termination + API key revocation), /Groups read-only, full RFC 7644 filter parser. I-14: POST /billing/upgrade (validates target plan, downgrade protection, seat/feature enforcement), GET /billing/plans. 24 files changed (10 new, 8 modified, 6 test count fixes). 49 new tests. Commit 48798de. Pushed to master. ~7,311 monorepo tests. CI/CD passed, API gateway healthy (110s uptime).
- **Known issues:** Shodan/GreyNoise API keys not set on VPS (enrichment degrades gracefully). Alert fan-out uses in-memory tenant registry. 2 pre-existing test files fail (batch-normalizer, fuzzy-dedupe-integration) due to vitest alias caching. VPS needs `prisma db push` (ScimToken model + SsoConfig model + User email verification fields + plan models + migration 0003). 1 pre-existing flaky test in shared-auth (password.test.ts unique salts).
- **Next tasks:** (1) Run `prisma db push` on VPS for ScimToken + SsoConfig + email verification + plan models. (2) Run seed script for 4 default plans. (3) Set TI_MFA_ENCRYPTION_KEY env var on VPS. (4) Command Center v2.1 — remaining I-items or Quota UI (billing/limits frontend). (5) Set Shodan/GreyNoise API keys on VPS.
