...
| Session 149 | 2026-09-25 | **No new issues.** Two clean deploys to master: PR #31 merge (run 36106947700) landed the health-recovery exec-bit fix, then PR #32 merge (run 36108011998) shipped the offboarding purge (tenant graph/search/cache deletion). Both green end-to-end; VPS at `d447962`; 32/32 healthy incl. `etip_user_management`. **Merge-order care worked:** PR #31 was merged first so PR #32's `git reset --hard` kept the scripts at mode 100755 (verified `git ls-tree master scripts/health-recovery.sh` = 100755). **Prevention reminder:** when two branches are open and one restores an exec bit / infra fix, always merge that one first, since every master push force-resets the VPS working tree. |
