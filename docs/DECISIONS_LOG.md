...
**Consequences:** 7 new Prisma models, 5 implementation phases (A1: schema+catalog+standards, A2: AI config+Bayesian confidence+EPSS, B: global pipeline+fuzzy dedup+MISP warninglists, C: enrichment+overlay+Shodan/GreyNoise+ATT&CK weighting, D: frontend+migration+explainability+ATT&CK heatmap). 27 total improvements (12 original + 15 standards-based: NATO Admiralty Code, CPE 2.3, STIX Sightings, Bayesian confidence, EPSS live, fuzzy dedup, MISP Warninglists, CWE chains, STIX confidence tiers, Shodan+GreyNoise, ATT&CK tactic weighting, AI graph extraction, confidence explainability, STIX import/export wizard, ATT&CK Navigator heatmap). Feature-flagged rollout. Full plan: docs/architecture/DECISION-029-Global-Processing-Plan.md
